...
/*    キ???ドアイコン(言語バ?の)の?示/非?示を切り替える関数。
 *※
 *    Text Service Framework と統合されている場合にはキ???ドのアイコンが
 *    ?示されるが、そうでない場合には IME ICON が利用されるようである。
 */
BOOL
_ShowKeyboardIcon (
    register BOOL        fShow)
{
    ...
    if (SUCCEEDED (pLangBarItemMgr->GetItem (c_guidKeyboardItemButton, &pItem))) {
        if (fShow) {
            ...
            /*    Text Service Framework と IME が統合されていない場合には、
             *    IME Icon が?示されてしまい、それが Button の1つと重なる
             *    ために、キ???ドアイコンを変更する。
             */
